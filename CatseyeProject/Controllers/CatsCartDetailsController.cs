using CatseyeProject.Context;
using CatseyeProject.Models.DTOs;
using CatseyeProject.Models;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using System.Collections.Generic;
using System.Linq;

namespace CatseyeProject.Controllers
{
    [Route("api/[controller]")]
    [ApiController]
    public class CatsCartDetailsController : ControllerBase
    {
        private readonly ApplicationDbContext _db;
        public CatsCartDetailsController(ApplicationDbContext db)
        {
            _db = db;
        }


        [HttpGet]
        [Route("GetCartDetails")]
        public IEnumerable<CatsCartDetails> Get()
        {
            var data = _db.tblCartDetails.ToList();
            return data;
        }


        [HttpGet]
        [Route("GetCartDetailsById")]
        public ActionResult<CatsCartDetails> GetById(int? id)
        {
            if (id == 0 && id == null)
            {
                return BadRequest(new RequestResult() { Result = false, Errors = new List<string>() { "Not Found" } });
            }
            var data = _db.tblCartDetails.FirstOrDefault(x => x.Id == id);
            if (data == null)
            {
                return BadRequest(new RequestResult() { Result = false, Errors = new List<string>() { "Not Found" } });
            }
            return Ok(data);
        }
    }
}
